...
<html>
  <head>
    <title>Registeration  | Massivesoft</title>
    <?php include '../include/header.php';?>
  </head>
<body style="background-color: #f9f9f9; font-family: 'Source Sans Pro', sans-serif;">
<?php require('config.php');

if( $user->is_logged_in() ){ header('Location: ../index.php'); }      //if logged in return to
...
if(isset($_POST['submit'])){
    ...
    if(!isset($error)){                                                                         //Go ahead with account creation
        ...
        <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
            <form role="form" method="post" action="" autocomplete="off">
                ...
                if(isset($error)){              //check for any errors
                    foreach($error as $error){echo '<div class="alert alert-danger" role="alert">'.$error.'</div>';}}
                if(isset($_GET['action']) && $_GET['action'] == 'joined'){                      //if action is joined show sucess
                    echo "<div class='alert alert-success' role='alert'><h2 class='bg-success'>Registration successful, please click to activate your account.</h2><div>";}?>
